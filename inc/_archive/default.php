...
function seo_warning() {
	...
	$message = __( 'You are blocking access to robots. You must go to your <a href="%s">Reading</a> settings and uncheck the box for Search Engine Visibility.', 'popin' );
...
function theme_localization () {
	...
	load_theme_textdomain( 'popin', get_template_directory() . '/languages' );
...
// Remove "Tags:" mention in the title
add_filter( 'get_the_archive_title', function ( $title ) {

    if( is_tag('Product Blog') ) {
...
function wp_admin_bar_valid_search_menu( $wp_admin_bar ) {
	...
	$form .= '<input type="submit" class="adminbar-button" value="' . __( 'Search', 'popin' ) . '"/>';
...
function theme_get_the_password_form() {
	...
	<p>' . __( 'This content is password protected. To view it please enter your password below:', 'popin' ) . '</p>

	<p><label for="' . $label . '">' . __( 'Password:', 'popin' ) . '</label> <input name="post_password" id="' . $label . '" type="password" size="20" /> <input type="submit" name="Submit" value="' . esc_attr__( 'Submit', 'popin' ) . '" /></p></form>
...
function post_terms( $value = '', $tax_name = '' ){
	...
	if ( $terms ) {
		...
		foreach( $terms as $term ) {
			...
			if ( $value == 'slug' ){ 
				...
				$output .= $space.$term->slug;

				$space = ' ';

			} else if ( $value == 'name' ){ 

				$output .= $space.'<a class="'. get_field( 'color', 'news-taxonomy_'.$term->term_id ) .'" href="javascript:void(0);">'. $term->name .'</a>';

				$space = ' ';

			} else {

				$output .= $comma.'<a class="'. get_field( 'color', 'news-taxonomy_'.$term->term_id ) .'" href="'. get_term_link( $term ) .'">'. $term->name .'</a>';
...
function theme_body_class($classes) {
	...
	if (is_array($classes)) {
		...
		if( !is_page_template() ) $classes[0] = ' inner';
...
function retina_image( $args = array() ){
	...
	if ( $image_id ) {
		...
		$large = wp_get_attachment_image_src( $image_id, $large_size );

		$small = wp_get_attachment_image_src( $image_id, $small_size ); 

		echo '

		<picture>

			<source srcset="'. $small[0] .'" media="(max-width: 767px)">

			<source srcset="'. $large[0] .'">

			<img src="'. $large[0] .'" alt="'. get_post_meta( $image_id, '_wp_attachment_image_alt', true) .'">
