	...
	function enqueue_hashed_files() {
		...
		foreach ($dist_files as $file) {		
			...
			if (pathinfo($file, PATHINFO_EXTENSION) === 'js') {
				...
				// Check for files containing "instapage" in it's filename.
				// This will prevent xxxx.instapage.js and xxxx.instapage.css files from being used.
				$instapageJsCss = strpos($fullName, 'instapage');

				if (($instapageJsCss === false) && $fullName !== 'analytics.js')  {
